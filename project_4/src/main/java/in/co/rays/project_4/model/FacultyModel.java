...
public class FacultyModel {
	...
	public void update(FacultyBean bean) throws ApplicationException, DuplicateRecordException{
		...
		try{
			...
			PreparedStatement stmt=conn.prepareStatement("Update st_faculty set first_name=?,last_name=?,gender=?,joining_date=?,qualification=?,email=?,mobile_no=?,college_id=?,college_name=?,course_id=?,course_name=?,subject_id=?,subject_name=?,created_by=?,modified_by=?,created_datetime=?,modified_datetime=? where id=?");
